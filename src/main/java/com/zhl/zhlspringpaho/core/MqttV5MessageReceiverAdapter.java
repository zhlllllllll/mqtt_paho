 ...
 * 接收消息
 */
public class MqttV5MessageReceiverAdapter extends AbstractMqttReceiverAdapter
    ...
    private Map<String, MqttListenerTemplate> topic2template = new HashMap<>();
    private MqttListenerTemplate defaultListener = new DefaultMqttListener();

    private List<MqttListenerTemplate> templates;

    public MqttV5MessageReceiverAdapter(String url, String clientId, String... topic) {
    ...
    public MqttV5MessageReceiverAdapter(MqttConnectionOptions connectionOptions, String clientId,
                                        String... topic) {
...
//        super.onInit();
        if (this.mqttClient == null) {
            try {
                ...
                throw new BeanCreationException("Cannot create 'MqttAsyncClient' for: " + this.getBeanName(), ex);
            }
        }
    ...
    public void messageArrived(String topic, MqttMessage mqttMessage) {
        ...
        try {
            ...
            // TODO: 2022/7/10 对topic进行解析，支持通配符 
            final MqttListenerTemplate mqttListenerTemplate = topic2template.get(topic);
            MqttListenerTemplate defaultListener = mqttListenerTemplate == null ? this.defaultListener : mqttListenerTemplate;
            defaultListener.messageProcessor(message, new MessageHeaders(headers));
    ...
    @Autowired
    public void setAllTemplates(List<MqttListenerTemplate> templates) {
    ...
    public void afterPropertiesSet() throws Exception {
        ...
        for (MqttListenerTemplate template : templates) {
            Class<?> targetClass = AopUtils.getTargetClass(template);
            //获取templates里面的MqttListener注解
            MqttListener classLevelListener = findListenerAnnotations(targetClass);
            final String connection = classLevelListener.connection();
            if (!connection.equals(beanName)) {
                //注解connection name是当前名称的，留下，其余跳过
                continue;
            }
            final String topic = classLevelListener.topic();
            if (topic2template.containsKey(topic)) {
                continue;
            }
            final QosType qos = classLevelListener.qos();
            this.addTopic(topic, qos.value());
            //留下的根据topic分组，组成map
            topic2template.put(topic, template);
        }
    }

    private MqttListener findListenerAnnotations(Class<?> clazz) {
        return AnnotatedElementUtils.findMergedAnnotation(clazz, MqttListener.class);
    }

    private class DefaultMqttListener implements MqttListenerTemplate {
        @Override
        public void messageProcessor(Message message, MessageHeaders headers) {
            logger.info(String.format("messageProcessor messageProcessor  content =%s ,message = %s, headers = %s", new String((byte[]) message.getPayload()), message, headers));
        }
    }
}
